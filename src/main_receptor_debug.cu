...
// Global log file handle for detailed logging
// This pointer is used across compilation units for writing detailed debug information
...
int main(int argc, char** argv) {
    ...
    // Initialize CUDA device before any GPU operations
    // Skip device 0 due to hardware issues, try devices 1-7
    cudaError_t cudaStatus = cudaSuccess;
    int selected_device = -1;

    for (int device_id = 0; device_id <= 7; device_id++) {
        cudaGetLastError();  // Clear any previous errors
        cudaStatus = cudaSetDevice(device_id);

        if (cudaStatus == cudaSuccess) {
            // Verify device is actually working by querying properties
            cudaDeviceProp prop;
            cudaStatus = cudaGetDeviceProperties(&prop, device_id);

            if (cudaStatus == cudaSuccess) {
                selected_device = device_id;
                std::cout << "[INFO] Using GPU " << device_id << ": " << prop.name << std::endl;
                break;
            }
        }
    }
    ...
    if (selected_device == -1) {
        std::cerr << Color::RED << "[ERROR] " << Color::RESET
                  << "Failed to initialize any CUDA device (tried devices 1-7)" << std::endl;
        std::cerr << "[INFO] Last error: " << cudaGetErrorString(cudaStatus) << std::endl;
        return 1;
    }

    std::cout << "[INFO] CUDA device " << selected_device << " initialized successfully" << std::endl;
